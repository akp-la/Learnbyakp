...
const countries = [
  { id: "in", code: "IN", name: "India", dialCode: "+91" },
  { id: "us", code: "US", name: "United States", dialCode: "+1" },
  { id: "uk", code: "UK", name: "United Kingdom", dialCode: "+44" },
  { id: "ca", code: "CA", name: "Canada", dialCode: "+1" }
];
...
function createPhoneNumber(countryCode) {
  switch (countryCode) {
    case "US":
    case "CA":
...
function buildMockMessages(number) {
  const otp = randomOtp();
  const altOtp = randomOtp();
  const now = Date.now();

  return [
    {
      id: `msg-${number.id}-1`,
      from: number.service || "Demo App",
      text: `Your verification code is ${otp}. Do not share this OTP with anyone.`,
      createdAt: new Date(now - 45 * 1000).toISOString()
    },
    {
      id: `msg-${number.id}-2`,
      from: "Secure Login",
      text: `Use code ${altOtp} to complete sign in.`,
      createdAt: new Date(now - 10 * 1000).toISOString()
    }
  ];
}

app.get("/api/temp-mobile/countries", (_req, res) => {
  res.json({ data: countries });
});

app.post("/api/temp-mobile/numbers", (req, res) => {
  const countryInput = String(req.body?.country || "IN").toUpperCase();
  const country = countries.find(
    (item) => item.code === countryInput || item.id.toUpperCase() === countryInput
  ) || countries[0];

  const service = String(req.body?.service || "General").trim() || "General";
  const id = `num-${Date.now()}-${randomDigits(4)}`;
  const sessionId = `sess-${randomDigits(10)}`;

  const number = {
    id,
    number: createPhoneNumber(country.code),
  ...
  const messages = buildMockMessages(number);
  numbers.unshift(number);
  messagesBySession.set(sessionId, messages);
  messages.forEach((item) => {
    messageIndex.set(item.id, item);
  });

  res.status(201).json({ data: number });
});

app.get("/api/temp-mobile/numbers/:sessionId/messages", (req, res) => {
  const sessionId = req.params.sessionId;
  const messages = messagesBySession.get(sessionId) || [];
  res.json({ data: messages });
});

app.get("/api/temp-mobile/messages/:messageId", (req, res) => {
  const message = messageIndex.get(req.params.messageId);
  if (!message) {
...
app.post("/api/temp-mobile/mock/inject", (req, res) => {
  const sessionId = String(req.body?.sessionId || "");
  const text = String(req.body?.text || "").trim();
  const from = String(req.body?.from || "Manual Test").trim() || "Manual Test";

  if (!sessionId || !messagesBySession.has(sessionId)) {
  ...
  const item = {
    id: `msg-${Date.now()}-${randomDigits(4)}`,
    from,
    text,
    createdAt: new Date().toISOString()
  };
